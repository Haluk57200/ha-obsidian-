// ha-obsidian — Media Premium Player V943
// FIX MASS config_entry_id obligatoire (MASS 2.5+)
...
// Mots-clés d'ambiance  suggestion "Playlist ambiance"
const _AMBIANCE_KEYWORDS = [
  "relax","chill","sport","workout","training","focus","study","etude","travail","work",
  "enfant","enfants","child","kids","bebe","bébé","dodo","sommeil","sleep","calme",
  "fete","fête","soiree","soirée","party","dance","danse",
  "techno","house","rock","pop","rap","jazz","classique","classical","metal",
  "reggae","electro","ambient","lounge","meditation","méditation","yoga",
  "cuisine","douche","massage","romantique","romance","noel","noël","christmas","disney"
];
...
class MediaPremiumPlayerV941 extends HTMLElement {
  ...
  room()  {
    const r = this.state(this.config.music_player_select,"Ampli");
    return ["Ampli","Cuisine","Maison"].includes(r) ? r : "Ampli";
  }
  roomInfo(room = this.room()) {
    ...
    return { id: c.denon_player, name: "Ampli", rgb: "255,143,179" };
  }
  activeTv() {
    const c = this.config;
...
async _playMassItem(item, entityId) {
  ...
  if (uri) {
    //  PAS de config_entry_id ici — play_media le refuse
    try {
      ...
      // Pour Maison : grouper la cuisine sur Ampli (HEOS)
...
async _loadQueue(entityId) {
  ...
  try {
    ...
    //  Méthode 1 : get_queue standard (current + next + count)
    ...
    //  Méthode 2 : tenter plusieurs message types WebSocket MASS
    if (items.length < 3) {
      ...
      for (const msg of attempts) {
        try {
          ...
          if (arr && arr.length > 2) {
            items = arr;
            console.log(`[Queue] ${arr.length} items récupérés via ${msg.type}/${msg.service || ""}`);
            break;
          }
        } catch (e) { /* type pas supporté, on continue */ }
      }
    }

    //  Méthode 2bis : essayer l'API HTTP MASS directe (port 8095)
    if (items.length < 3) {
      try {
        const mcid = this.attr(targetId, "media_content_id", "") || "";
        const entityPic = this.attr(targetId, "entity_picture", "") || "";
        // extraire host:port depuis entity_picture (ex: http://192.168.X.X:8095/...)
    ...
    //  Méthode 3 : fallback current + next
...
_renderQueuePanel() {
  ...
  //  On ne montre QUE les titres à venir (pas le current, déjà affiché dans le player)
  ...
  const countText = remaining > upcoming.length
    ? `${upcoming.length} affichés · ${Math.max(remaining, total)} en file`
    : `${upcoming.length} à suivre`;
  ...
  //  Hint quand on a moins d'items que ce que MASS prétend avoir
  const hint = (remaining > upcoming.length && upcoming.length > 0)
    ? `<div class="q-hint">API MASS limitée — ${total - upcoming.length - 1} titres masqués</div>`
    : "";

  const body = upcoming.length > 0
    ? upcoming.map((it,i) => renderRow(it, String(i+1))).join("") + hint
    : `<div class="q-empty">Aucun titre à suivre</div>`;

  return `<div class="queue-panel">
    <div class="q-header">
      <span class="q-ttl">${this.ico("queue",14)} À suivre</span>
  ...
  // via music_assistant.play_media — sans passer par le script HA.
  async _launchIntent(kind, query) {
    ...
    // Étape 1 : essayer le cache de recherche pour avoir un vrai URI (avec logo)
    ...
    if (directHit) {
      console.log(`[intent ${kind}] hit cache  URI direct`, directHit.uri || directHit.item_id);
      return this._playMassItem(directHit, target);
    }

    // Étape 2 : appel direct play_media avec la query comme media_id
    // MASS résout via ses providers (Radio Browser pour les radios)
    const payload = {
      entity_id: targetId,
      media_id:  query.trim(),
    ...
    try {
      ...
      console.warn(`[intent ${kind}] play_media a échoué`, err?.message || err);
  ...
  // Détecte si la query correspond à une radio FR ou un mot d'ambiance.
  ...
  _detectIntent(query) {
    ...
    if (/^radio\+\/.test(q) || /\$/.test(q) || /^fm\+\/.test(q)) {
      return { kind: "radio", label: query, query };
    }
    // Match radio par mots-clés
  ...
  _loadDiscover(entityId) {
    ...
    this.call("script","turn_on",
      { entity_id: "script.lastfm_artistes_similaires",
  ...
  _parseDiscover() {
    ...
    return arr.slice(0, 6).map(a => {
      ...
      // Last.fm renvoie souvent des URLs cassées ou un placeholder étoile.
      // On détecte ces cas et on les vide pour utiliser un fallback propre.
  ...
  // Lance en arrière-plan une recherche Deezer pour les artistes sans image
  ...
  _colorForName(name) {
    // Hash simple  couleur du dyn palette
    ...
    if (!items.length) {
      return `<div class="disc-panel">
        <div class="disc-header">
          <span class="disc-ttl">${this.ico("sprk",13)} À découvrir${artist ? ` · ${this.esc(artist)}` : ''}</span>
          <span class="disc-close" data-action="toggle-discover"></span>
        </div>
        <div class="disc-empty">Chargement des artistes similaires...</div>
      </div>`;
    }
    // Déclencher l'enrichissement Deezer en arrière-plan pour les images manquantes
    if (items.some(it => !it.img)) this._enrichDiscoverImages();

    return `<div class="disc-panel">
      <div class="disc-header">
        <span class="disc-ttl">${this.ico("sprk",13)} Similaires à ${this.esc(artist)}</span>
        <span class="disc-close" data-action="toggle-discover"></span>
      </div>
      <div class="disc-grid">
        ${items.map(it => {
  ...
  _renderHistoryPanel() {
    ...
    if (!items.length) {
      return `<div class="hist-panel">
        <div class="hist-header">
          <span class="hist-ttl">${this.ico("hist",13)} Récemment écoutés</span>
          <span class="hist-close" data-action="toggle-history"></span>
        </div>
    ...
    return `<div class="hist-panel">
      <div class="hist-header">
        <span class="hist-ttl">${this.ico("hist",13)} Récemment écoutés · ${items.length}</span>
        <span class="hist-clear" data-action="history-clear">Effacer </span>
      </div>
      <div class="hist-list">
        ${items.map((it,i) => {
...
//
// 1) _playMassItem — payload minimal, MASS infère depuis le URI
//
async _playMassItem(item, entityId) {
  const uri     = item.uri || item.item_id || item.media_content_id || "";
  const rawType = item._cat || item.media_type || item.type || "";
  //  MASS search renvoie en pluriel ("tracks","radios"...), play_media veut le singulier
  ...
  if (uri) {
    // Payload minimal — pas d'enqueue / radio_mode pour les radios :
    ...
    try {
      await this._hass.callService("music_assistant", "play_media", payload);
      // Pour Maison : grouper la cuisine sur Ampli (HEOS) — sauf radio (HEOS gère mal)
    ...
    // Fallback 1 : retry sans media_type (MASS infère depuis URI)
    if (type) {
      try {
  ...
  // Dernier recours : script HA via input_text (qui détecte radio/ambiance)
  const name = item.name || item.title || "";
  if (name && c.search_text && c.script_launch) {
    await this.call("input_text","set_value",{
      entity_id: c.search_text,
  ...
  connectedCallback() {
    ...
    const _rafBar = () => {
      ...
      //  MAJ aussi du texte temps courant
...
async _getMassEntryId() {
  ...
  _renderDropdown(items) {
    ...
    dd.innerHTML = `
      ...
      <div class="srch-header">
        <span class="srch-count">${items.length} résultat${items.length>1?"s":""}${intent?" + suggestion":""}</span>
  ...
  _buildIntentItem(intent) {
    const ico   = intent.kind === "radio" ? "" : "";
    const title = intent.kind === "radio"
      ? `Lancer ${intent.label} en radio`
      : `Playlist ambiance · ${intent.label}`;
    const sub   = intent.kind === "radio"
      ? "Music Assistant · radio_mode"
      : "Music Assistant · playlist générée";
  ...
  cssText() { return `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /*
       RESPONSIVE — Mode tablette/téléphone
        */
    /*  Tablette (< 720px) : un peu plus compact  */
    ...
    @media (max-width: 480px) {
      ...
      /* Queue / history / discover : hauteur réduite */
    ...
    /*  Très petit téléphone (< 380px) : ultra-compact  */
  ...
  renderMusic() {
    ...
    // En mode Maison (groupe Ampli+Cuisine), le maître audio est Ampli
    //  on lit les métadonnées sur Ampli.
    // En mode Cuisine seul, Cuisine joue indépendamment, on lit sur Cuisine.
    // En mode Ampli, on lit sur Ampli (masterId).
    ...
    const sub      = artist ? `${artist}${album && album !== title ? " · "+album : ""}` : (this.state(mediaId,"off")==="playing" ? "Music Assistant" : "Tidal · Music Assistant");
    ...
    const volAmpli = this.attr(c.denon_player,"volume_level") != null ? Math.round(this.attr(c.denon_player,"volume_level")*100) : 0;
    const volCuisine = this.attr(c.cuisine_player,"volume_level") != null ? Math.round(this.attr(c.cuisine_player,"volume_level")*100) : 0;
    //  Détecter la source réelle depuis media_content_id (radiobrowser/tidal/spotify/…)
    ...
    return `
      ...
      <div class="layout">
        ...
        <div class="dock">
          <div class="room-vol">
            <button class="r-pill ${room==="Ampli"?"active":""}"   data-action="room-ampli">${this.ico("music",10)} Ampli</button>
            ...
            ${isMaison ? `
              <div class="vol-multi">
                <div class="vol-room">
                  ...
                  <div class="vol-hit" data-action="vol-seek-denon"><div class="vol-rail"><div class="vol-bar" style="width:${volAmpli}%"></div></div></div>
                  <span class="vr-val">${volAmpli}</span>
                </div>
                <div class="vol-room">
          ...
          <div class="transport">
            <div class="t-left">
              ...
              <button class="t-btn side-ico" data-action="transfer" title="Transférer">${this.ico("xfer",17)}</button>
              <button class="t-btn side-ico${shuffleOn?' on':''}" data-action="shuffle" title="Aléatoire">${this.ico("shuf",16)}</button>
              <button class="t-btn side-ico${repeatMode!=='off'?' on':''}" data-action="repeat" title="${repeatMode==='one'?'Répéter un':repeatMode==='all'?'Répéter tout':'Répéter'}">${this.ico(repeatMode==='one'?"rep1":"repa",16)}</button>
            </div>
            ...
            <div class="t-right">
              ...
              <button class="t-btn side-ico${this._discoverOpen?' on':''}" data-action="toggle-discover" title="À découvrir">${this.ico("sprk",16)}</button>
              <button class="t-btn stop" data-action="stop">${this.ico("stop",17)}</button>
              ${room === "Ampli" ? `<button class="t-btn pwr" data-action="denon-power">${this.ico("pwr",17)}</button>` : ""}
  ...
  _parseTiviContent() {
    ...
    const EP_RE      = /(?:[Ss](\{1,2})[xXeE](\{1,2})|(\{1,2})[xX](\{1,2}))/;
    const mAdbFull   = adb.match(/text="[|]?[A-Z]{2,6}[|]?\(.+?)"/);
    const adbRaw     = mAdbFull ? mAdbFull[1].trim() : "";

    //  Détecter pattern épisode (S01E02, 1x02…)
    ...
    //  1) Pattern épisode quelque part  SÉRIE (peu importe le reste)
    if (hasAnyEp) {
      ...
      const rawTitle = vodTitre ||
        adbRaw.replace(/\\{1,2}[xXeE]\{1,2}.*/,"").replace(/[|][A-Z]+[|]\/,"").trim();
      return { type:"serie", rawTitle, season, episode };
    }

    //  2) tmdb_titre rempli MAIS différent du programme EPG  FILM (VOD)
    //     Quand tu regardes un film via TiviMate, tmdb_titre = film,
    //     tivimate_programme = ce qui passe sur la chaîne (EPG figé/stale).
    //     Si les deux sont différents  l'utilisateur regarde une VOD.
    //     Si les deux matchent  TMDB a juste enrichi le programme live.
    ...
    //  3) Signal LIVE : logo de chaîne reconnu OU horaire au format HH:MM
    const isHHMM = tiviTime && /\{1,2}[h:.]\{2}/.test(tiviTime);
    if (tiviLogo || isHHMM) {
      return { type:"live", rawTitle: tiviChaine };
    }

    //  4) Pas de logo, pas d'horaire, mais tmdb_titre rempli  vraie VOD
    if (vodTitre) {
      return { type:"film", rawTitle: vodTitre };
    }

    //  5) Fallback : LIVE avec ce qu'on a
  ...
  renderTV() {
    ...
    if (tiviIsVodSerie) {
      const rawClean = tiviContent.rawTitle.replace(/\\|[A-Z]+\\|\/g,"").replace(/[Ss]\+[xXeE]\+.*/,"").trim();
      ...
      const rawClean = tiviContent.rawTitle.replace(/\\|[A-Z]+\\|\/g,"").trim();
  ...
  bind() {
    this.shadowRoot.querySelectorAll("[data-action]").forEach(el => {
      ...
      if (a === "vol-seek" || a === "vol-seek-denon" || a === "vol-seek-cuisine") {
        //  Drag-to-set volume avec feedback visuel immédiat
        const _getTargets = () => {
        ...
        const _updateBar = (p) => {
          ...
          // Si on est en mode multi-pièce, met aussi à jour le label numérique
  ...
  handle(a, el) {
    ...
    if (a === "room-ampli" || a === "room-cuisine" || a === "room-maison") {
      const roomName = a==="room-ampli" ? "Ampli"
    ...
    if (a === "lastfm-search") {
      ...
      if (!artist) return;
      return this.call("script","turn_on",{entity_id:"script.lastfm_top_titres_artiste",variables:{artiste:artist}});
    }
    if (a === "play-track" && el) {
